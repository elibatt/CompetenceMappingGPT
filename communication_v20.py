...
for i in tqdm(range(1, 19), desc="Testing phase"):
    ...
    scoring = " Give me a value between 1 and 5. "
    scoring_binary = " Give me 1 if he was, 0 otherwise. "
    output_rules = (
        "You have to answer only with the score for each question like this: Score; Score; Score; Score;"
        + additional_notes
    )
    ...
    print(response)
    value1 = response.split(";")[0]
    value2 = response.split(";")[1]
    value3 = response.split(";")[2]
    value4 = response.split(";")[3]
    values.append(int(value1))
    values.append(int(value2))
    values.append(int(value4))
    min_value = 1
    max_value = 5
    normalized_values = [(val - min_value) / (max_value - min_value) for val in values]
    normalized_values.append(int(value3))

    overall = sum(np.array(normalized_values))
    target_overall = sum(np.array(expected_values[i - 1]))

    row = pd.DataFrame(
        {
            "TRANSCRIPTION_NUMBER": [i],
            "KINDNESS": [normalized_values[0]],
            "ABILITY_TO_EXPLAIN": [normalized_values[1]],
            "PITCH_SALE": [normalized_values[3]],
            "CUSTOMER_SATISTFACTION": [normalized_values[2]],
...
df.to_csv("data/results_communication_competences_v2.0.csv", index=False)
